...
interface IButtonDrawerContentProps {
  buttonName: string;
...
const ButtonDrawerContent = memo(({ buttonName, onToggle, onShowBeardStyle }: IButtonDrawerContentProps) => {
  const classes = useStyles();
  const [isVideoMaxSize, setVideoMaxSize] = useState(false);

  const buttons = useBoundStore(state => state.buttons);
  const beardStyles = useBoundStore(state => state.product?.beardStyles);;
  ...
  const buttonContent = useMemo(() => getButtonContent(buttonName, buttons), [buttonName, buttons])
  ...
  // FIXME SwipeableDrawer is not correct
  return (
    <SwipeableDrawer
      anchor="bottom"
      hideBackdrop
    ...
    >
      <Grid sx={theme => ({
        ...
        ...theme.arPageStyles?.buttonDrawerContent.root
      })}>
...
export default ButtonDrawerContent;
